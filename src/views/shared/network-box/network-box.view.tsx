...
import { HYPERCHAIN_SUPPORT_URL } from "src/constants";
...
export const NetworkBox: FC = () => {
  ...
  const successMsg: Message = {
    text: `${hyperchainZkEVMChain.name} network successfully added`,
    type: "success-msg",
  };

  const onAddNetwork = (): void => {
    setIsAddNetworkButtonDisabled(true);
    addNetwork(hyperchainZkEVMChain)
      .then(() => {
        callIfMounted(() => {
        ...
        <ul className={classes.list}>
          <li className={classes.listItem}>
            <Typography type="body2">
              RPC URL: {hyperchainZkEVMChain.provider.connection.url}
            </Typography>
          </li>
          <li className={classes.listItem}>
            <Typography type="body2">Chain ID: {hyperchainZkEVMChain.chainId}</Typography>
          </li>
          <li className={classes.listItem}>
            <Typography type="body2">
              Currency symbol: {hyperchainZkEVMChain.nativeCurrency.symbol}
            </Typography>
          </li>
          <li className={classes.listItem}>
            <Typography type="body2">
              Block explorer URL:{" "}
              <ExternalLink href={hyperchainZkEVMChain.explorerUrl}>
                {hyperchainZkEVMChain.explorerUrl}
              </ExternalLink>
            </Typography>
          </li>
          <li className={classes.listItem}>
          ...
          <button
            ...
            disabled={
              ...
              (isAsyncTaskDataAvailable(connectedProvider) &&
                connectedProvider.data.chainId === hyperchainZkEVMChain.chainId)
            }
