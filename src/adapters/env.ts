...
interface Env {
  ...
  VITE_HYPERCHAIN_ZK_EVM_RPC_URL: string;
...
const envToDomain = ({
  ...
  VITE_HYPERCHAIN_ZK_EVM_RPC_URL,
  ...
  const hyperchainZkEVMNetworkId = z.coerce.number().positive().parse(VITE_HYPERCHAIN_ZK_EVM_NETWORK_ID);
...
const envParser = StrictSchema<Env, domain.Env>()(
  z
    .object({
      ...
      VITE_HYPERCHAIN_ZK_EVM_RPC_URL: z.string().url(),
